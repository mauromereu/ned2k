...
class Conn extends EventEmitter {
  ...
  _onClientProcessEndHandler(info) {
    ...
    // switch add { } to prevent declared variables
    switch (headerPacket.optype) {
      case HelloPacket.OPCODE:
        {
          let hello = new HelloPacket();
          ...
          let helloAnswer = new HelloAnswerPacket(this._conn._session);
          this.write(combinePackets(header, helloAnswer));
          break;
        }
      case HelloAnswerPacket.OPCODE:
        {
          let helloAnswer = new HelloAnswerPacket();
