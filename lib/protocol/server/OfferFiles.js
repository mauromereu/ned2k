...
class OfferFiles extends UsualPacket {
  ...
  encode() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      fileBuf.writeUInt32LE(fileItem.ip || 0x0);
      fileBuf.writeUInt16LE(fileItem.port || 4662);

      let tagList = Tag.encode2Buffer([
        { type: Tag.TYPE.TT_STRING, opcode: CT.FILENAME, data: fileItem.name },
        { type: Tag.TYPE.TT_UINT32, opcode: CT.FILESIZE, data: fileItem.size },
        { type: Tag.TYPE.TT_STRING, opcode: CT.FILETYPE, data: fileItem.type }
      ]);
