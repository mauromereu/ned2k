...
class SearchResults extends UsualPacket {
  ...
  encode() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let buffer = Buffer.alloc(STABLE_SIZE);
  ...
  decode(buffer) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let fileInfo = Tag.toKVObject(tagList, CT);
      this.files.push({
        hash: hash,
        ip: ip,
        port: port,
        ...
        type: fileInfo.filetype || Constant.FILETYPE.ANY,
        sources: fileInfo.sources || 0,
        complsrc: fileInfo.complsrc || 0
      });

      let tagsLen = tagList.map((tag) => {
      ...
      buffer = buffer.slice(STABLE_SIZE + 4 + tagsLen);
    }
  }
